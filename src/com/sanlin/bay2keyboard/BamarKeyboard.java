...
public class BamarKeyboard extends BKeyboard {
	...
	private void handelMyanmarWordDelete(InputConnection ic) {
		...
		while (!(isConsonant(current) || Bay2IME.isWordSeparator(current))// or
																		// Word
				// separator
	...
	private boolean getFlagMedial(InputConnection ic) {
		...
		while (!(isConsonant(current) || Bay2IME.isWordSeparator(current))
